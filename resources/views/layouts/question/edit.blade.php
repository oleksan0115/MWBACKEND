    ...
    <div class="content">
      <div class="invoice-wrapper rounded border bg-white py-5 px-3 px-md-4 px-lg-5 mb-6">
        ...
        <div class="d-flex">
          ...
          <h3 class="font-weight-bold ml-5 pl-5">Edit Initial Question</h3>
        </div>
        <div class="row pt-5">
          @forelse($Questions as $key => $Question)
            <form action="{{ route('backend.initialQuestionUpdate') }}" method="POST"class="product_table">
              @csrf
              <input type="hidden" value="{{$Question->id}}" name="quetion_id">
              <table width="633" border="0" cellpadding="3" cellspacing="3">
                ...
                <tr>
                  ...
                  <td width="458" align="left" valign="top">
                    <textarea name="quetion_name" id="txt_cat_desc" cols="50" rows="5" required>{{ $Question->question }}</textarea>
                  </td>
                </tr>
                <tr>
                    ...
                    <select id="txt_cat_desc" name="quetion_cat" required>
                      @forelse($QuestionCategories as $key => $QuestionCategory)
                        ...
                        if($Question->question_category_id == $QuestionCategory->id){
                          echo '<option selected value="'.$QuestionCategory->id.'">'.$QuestionCategory->category_name.'</option>';
                        } else {
                          echo '<option value="'.$QuestionCategory->id.'">'.$QuestionCategory->category_name.'</option>';
                        }
                        ?>
                      @empty
