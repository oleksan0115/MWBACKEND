...
class ProductController extends APIController
{
  ...
  public function addProduct(Request $request)
  {
    ...
    if (!empty($request['chkowner'])) {
      $chkowner = $request['chkowner'];
    } else {
      $chkowner = '';
    }
    if (!empty($request['chkstatus'])) {
      $chkstatus = $request['chkstatus'];
    } else {
      $chkstatus = '';
    }
    if (!empty($request['chkemojis'])) {
      $chkemojis = $request['chkemojis'];
    } else {
      $chkemojis = '';
    }
    if (!empty($request['chkisauction'])) {
      $chkisauction = $request['chkisauction'];
    } else {
      $chkisauction = '';
    }
    if (!empty($request['tboxstartdate'])) {
      $product_startdate = $request['tboxstartdate'];
    } else {
      $product_startdate = '';
    }
    if (!empty($request['tboxenddate'])) {
      $product_enddate = $request['tboxenddate'];
    } else {
      $product_enddate = '';
    }
    if (!empty($request['tboxinitialcredits'])) {
      $product_initialcredits = $request['tboxinitialcredits'];
    } else {
      $product_initialcredits = '';
    }
    if (!empty($request['tbox_active_datetime'])) {
      $product_active_datetime = $request['tbox_active_datetime'];
    } else {
      $product_active_datetime = '';
    }
...
// echo $pulse_updated;
    ...
    if ($rdo_newstype_free == null) {
      $rdo_newstype_free = 0;
    } else {
      $rdo_newstype_free = 1;
    }
    if ($rdo_newstype_paid == null) {
      $rdo_newstype_paid = 0;
    } else {
      $rdo_newstype_paid = 1;
    }
    if ($rdo_newstype_normail == null) {
      $rdo_newstype_normail = 0;
    } else {
      $rdo_newstype_normail = 1;
    }
